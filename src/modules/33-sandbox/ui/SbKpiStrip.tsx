...
interface SbKpiStripProps {
  kpis: KpiCard[];
  className?: string;
}

export function SbKpiStrip({ kpis, className }: SbKpiStripProps) {
  const { locale } = useApp();
    ...
    <div className={cn('grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4', className)}>
      {kpis.map((kpi) => (
        <Link
          ...
          href={kpi.link}
          className={cn(
            'group bg-white/70 backdrop-blur-sm rounded-2xl border p-4 hover:shadow-lg hover:shadow-indigo-500/10 transition-all duration-200',
        ...
        >
          ...
          <div className="text-xs text-stone-500 group-hover:text-indigo-600 transition-colors">
            {kpi.label[locale]}
          </div>
          {kpi.trend && (
            <div className={cn(
