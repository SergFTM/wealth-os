...
interface CalAiPanelProps {
  eventId?: string;
  onDraftAgenda?: () => Promise<AiResult>;
  onSummarizeNotes?: () => Promise<AiResult>;
  onExtractActions?: () => Promise<AiResult>;
  className?: string;
}

export function CalAiPanel({
  eventId,
  onDraftAgenda,
  onSummarizeNotes,
  onExtractActions,
  ...
  const actions = [
    {
      key: 'draftAgenda',
      label: 'Черновик повестки',
      description: 'Сгенерировать повестку на основе типа встречи',
      icon: (
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
        </svg>
      ),
      handler: onDraftAgenda,
    },
    {
      key: 'summarizeNotes',
      label: 'Суммаризация заметок',
      description: 'Создать краткое резюме заметок встречи',
      icon: (
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        </svg>
      ),
      handler: onSummarizeNotes,
    },
    {
      key: 'extractActions',
      label: 'Извлечь action items',
      description: 'Найти задачи и поручения в заметках',
      icon: (
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
      ),
      handler: onExtractActions,
    },
  ];

  const handleAction = async (key: string, handler?: () => Promise<AiResult>) => {
    if (!handler) return;
    ...
    <div className={cn("bg-white/80 backdrop-blur-sm rounded-xl border border-stone-200/50", className)}>
      <div className="px-4 py-3 border-b border-stone-200/50">
        ...
        <p className="text-xs text-stone-500 mt-1">
          Автоматизация подготовки и документирования встреч
        </p>
      </div>
      ...
      <div className="p-4 space-y-3">
        {actions.map((action) => (
          <button
            ...
            disabled={loading !== null || !action.handler}
            className={cn(
