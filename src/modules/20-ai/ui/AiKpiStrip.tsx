...
interface AiKpiStripProps {
  items: KpiItem[];
}

const iconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  aiEventsToday: Sparkles,
  narratives7d: FileText,
  draftsPending: Edit3,
  triageOpen: ListTodo,
  feedbackNegative: ThumbsDown,
  clientSafeCount: Shield,
  missingSources: AlertCircle,
  guardrailBlocks: ShieldAlert,
};
...
export function AiKpiStrip({ items }: AiKpiStripProps) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
      {items.map((item) => {
        ...
        const content = (
          <div
            className={`${colors.bg} rounded-xl p-4 border border-stone-200/50 hover:shadow-md transition-all cursor-pointer`}
          >
