...
import { MdAccountDetail } from '@/modules/46-mdm/ui/MdAccountDetail';

export default function MdmAccountPage() {
  const params = useParams();
  const id = params.id as string;

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: accounts = [], update } = useCollection('mdmAccounts') as { data: any[]; update: (id: string, data: any) => Promise<any> };
  ...
  const handleOverrideField = async (field: string, value: unknown) => {
    ...
    if (newValue !== null) {
      const overrides = account.overridesJson ? { ...account.overridesJson } : {};
      overrides[field] = {
        value: newValue,
      ...
      const chosen = { ...account.chosenJson };
  ...
  const handleAcceptSource = async (field: string, sourceSystem: string) => {
    const source = account.sourcesJson?.find((s: { sourceSystem: string }) => s.sourceSystem === sourceSystem);
      ...
      <div className="border-b border-stone-200/50 bg-white/50 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center gap-4">
            <Link href="/m/mdm/list?tab=accounts">
            ...
            <h1 className="text-xl font-semibold text-stone-800">Карточка счета</h1>
          </div>
        </div>
      </div>
