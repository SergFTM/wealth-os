...
import { MdEntityDetail } from '@/modules/46-mdm/ui/MdEntityDetail';
...
export default function MdmEntityPage() {
  ...
  const { data: entities = [], update } = useCollection('mdmEntities') as { data: any[]; update: (id: string, data: any) => Promise<any> };
  ...
  const handleOverrideField = async (field: string, value: unknown) => {
    ...
    if (newValue !== null) {
      const overrides = entity.overridesJson ? { ...entity.overridesJson } : {};
      overrides[field] = {
        value: newValue,
      ...
      const chosen = { ...entity.chosenJson };
  ...
  const handleAcceptSource = async (field: string, sourceSystem: string) => {
    const source = entity.sourcesJson?.find((s: { sourceSystem: string }) => s.sourceSystem === sourceSystem);
      ...
      <div className="border-b border-stone-200/50 bg-white/50 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center gap-4">
            <Link href="/m/mdm/list?tab=entities">
            ...
            <h1 className="text-xl font-semibold text-stone-800">Карточка сущности</h1>
          </div>
        </div>
      </div>
