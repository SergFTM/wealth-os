# M19 Reporting Studio and Exports

## 1. Назначение модуля
Конструктор отчётов, пакеты, экспорт и расписания.
...
- Multi-format export (PDF, Excel, web)
...
**ReportTemplate**
- Поля: id, name, type, sections[], parameters[], format, createdBy
- Связи: через objectRef / foreignKey

**ReportPack**
- Поля: id, name, clientId, period, status, sections[], approvedBy, publishedAt
- Связи: через objectRef / foreignKey
...
- `POST /api/actions/m19/<actionKey>`
  - Вход: payload JSON
...
## 13. Demo Data
### Seed данные
- Файл: `src/db/seed/seed-module-19.json`
- Коллекции: `reportTemplates`, `reportPacks`, `reportPackSections`, `reportExports`, `reportShares`, `reportLibraryItems`
...
- **Sources:** All modules (data sources)
- **Consumers:** M55 Portal, M52 Packs
