# M30 Capital Calls and Distributions Operations

## 1. Назначение модуля
Операционное управление capital calls и distributions для PE/VC.
...
**Value outcomes:**
- Automated capital call notice processing
- LP allocation waterfall calculations
- Distribution processing with tax lot integration
...
- `callNotices`
- `allocationWaterfalls`
- `auditEvents` (обязательно)

### Сущности (логическая модель)
**CapitalCall**
- Поля: id, fundId, callDate, totalAmount, lpAllocations[], dueDate, status, noticeRef
- Связи: через objectRef / foreignKey

**CapitalDistribution**
- Поля: id, fundId, distributionDate, totalAmount, type, lpAllocations[], status
- Связи: через objectRef / foreignKey
...
- `POST /api/actions/m30/<actionKey>`
  - Вход: payload JSON
...
## 13. Demo Data
### Seed данные
- Файл: `src/db/seed/seed-module-30.json`
- Коллекции: `capitalCalls`, `capitalDistributions`, `callNotices`, `allocationWaterfalls`
...
- **Sources:** M04 Private Markets
- **Consumers:** M02 GL, M20 Liquidity
- **Governance:** M07 Workflow
