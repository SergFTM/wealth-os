# M02 General Ledger and Accounting

## 1. Назначение модуля
Ведение главной книги для семейных структур с поддержкой IFRS/GAAP.
...
### Маршруты
- `/m/general-ledger`
- `/m/general-ledger/list`
- `/m/general-ledger/entry/[id]`
...
**JournalEntry**
- Поля: id, date, entityId, debitAccount, creditAccount, amount, currency, memo, status, sourceRef
- Связи: через objectRef / foreignKey

**ChartOfAccount**
- Поля: id, code, name, type, parentId, entityId, isActive
- Связи: через objectRef / foreignKey
...
- `POST /api/actions/m02/<actionKey>`
  - Вход: payload JSON
...
## 13. Demo Data
### Seed данные
- Файл: `src/db/seed/seed-module-02.json`
- Коллекции: `chartOfAccounts`, `journalEntries`, `trialBalance`, `periodCloses`
...
- **Consumers:** M05 Reporting, M13 Fees
