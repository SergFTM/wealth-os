# M48 Exception Center

## 1. Назначение модуля
Единый центр обработки исключений из всех модулей.
...
**Value outcomes:**
- Unified exception queue aggregating all modules
- Automated triage with configurable rules
- SLA tracking with escalation for unresolved exceptions
...
**Exception**
- Поля: id, source, module, type, severity, description, status, assignee, slaDeadline, resolvedAt
- Связи: через objectRef / foreignKey

**ExceptionRule**
- Поля: id, name, sourceModule, condition, autoAssignee, priority, active
- Связи: через objectRef / foreignKey
...
- `POST /api/actions/m48/<actionKey>`
  - Вход: payload JSON
...
## 13. Demo Data
### Seed данные
- Файл: `src/db/seed/seed-module-48.json`
- Коллекции: `exceptions`, `exceptionRules`, `exceptionClusters`, `exceptionSlaPolicies`
...
- **Sources:** M39 DQ, M21 Reconciliation, M14 Integrations
- **Consumers:** M22 Task Board, M38 Audit
