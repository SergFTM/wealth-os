# M22 Task Board and SLA Operations

## 1. Назначение модуля
Единый центр задач, очередей и SLA мониторинга.
...
**Value outcomes:**
- Unified task board across all modules
- SLA policy enforcement with auto-escalation
...
**Task**
- Поля: id, title, module, assignee, queue, priority, dueDate, slaDeadline, status, escalationLevel
- Связи: через objectRef / foreignKey

**SlaPolicy**
- Поля: id, name, module, priority, responseTime, resolutionTime, escalationRules[]
- Связи: через objectRef / foreignKey
...
- `POST /api/actions/m22/<actionKey>`
  - Вход: payload JSON
...
## 13. Demo Data
### Seed данные
- Файл: `src/db/seed/seed-module-22.json`
- Коллекции: `tasks`, `escalations`, `slaPolicies`, `complianceTasks`
