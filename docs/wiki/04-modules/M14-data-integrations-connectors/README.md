# M14 Data Integrations and Connectors

## 1. Назначение модуля
Центр интеграций: коннекторы к банкам, кастодианам, market data.
...
**Value outcomes:**
- Pre-built connectors for major custodians and banks
- Configurable field mapping and transformation
- Automated sync with error handling and retry
...
**Connector**
- Поля: id, name, type, provider, status, lastSync, errorCount, config
- Связи: через objectRef / foreignKey

**SyncJob**
- Поля: id, connectorId, schedule, lastRun, nextRun, status, recordsProcessed, errors
- Связи: через objectRef / foreignKey
...
- `POST /api/actions/m14/<actionKey>`
  - Вход: payload JSON
...
## 13. Demo Data
### Seed данные
- Файл: `src/db/seed/seed-module-14.json`
- Коллекции: `connectors`, `connectorCredentials`, `mappings`, `syncJobs`, `syncRuns`, `connections`
