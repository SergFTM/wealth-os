# M21 Reconciliation and Close

## 1. Назначение модуля
Сверка позиций, закрытие периодов, IBOR/ABOR reconciliation.
...
**Value outcomes:**
- Automated position reconciliation (IBOR vs ABOR)
- Bank and custodian statement matching
- Period close checklist with audit trail
...
### Маршруты
- `/m/reconciliation`
- `/m/reconciliation/list`
- `/m/reconciliation/rec/[id]`
...
**Reconciliation**
- Поля: id, type, sourceA, sourceB, asOf, status, matchCount, breakCount, completedAt
- Связи: через objectRef / foreignKey

**ReconciliationBreak**
- Поля: id, recId, field, valueA, valueB, difference, status, resolution
- Связи: через objectRef / foreignKey
...
- `POST /api/actions/m21/<actionKey>`
  - Вход: payload JSON
...
## 13. Demo Data
### Seed данные
- Файл: `src/db/seed/seed-module-21.json`
- Коллекции: `reconciliations`, `reconciliationBreaks`, `closeChecklists`, `closePeriods`
...
- **Sources:** M14 Integrations
- **Consumers:** M02 GL, M38 Audit
- **Governance:** M39 Data Quality
