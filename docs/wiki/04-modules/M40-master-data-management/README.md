# M40 Master Data Management

## 1. Назначение модуля
Управление мастер-данными: golden records, дедупликация.
...
**Value outcomes:**
- Single golden record per entity across systems
- Automated duplicate detection and merge suggestions
- Data steward workflow for manual resolution
...
**MdmEntity**
- Поля: id, type, goldenRecord, sourceRecords[], confidence, lastMerge, stewardStatus
- Связи: через objectRef / foreignKey

**MdmDuplicate**
- Поля: id, entityType, recordA, recordB, matchScore, status, resolvedBy
- Связи: через objectRef / foreignKey
...
- `POST /api/actions/m40/<actionKey>`
  - Вход: payload JSON
...
## 13. Demo Data
### Seed данные
- Файл: `src/db/seed/seed-module-40.json`
- Коллекции: `mdmEntities`, `mdmPeople`, `mdmAssets`, `mdmAccounts`, `mdmDuplicates`, `mdmMergeJobs`, `mdmRules`, `mdmStewardQueue`
...
- **Consumers:** All modules (master data)
