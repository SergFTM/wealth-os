# M06 Payments and Bill Pay

## 1. Назначение модуля
Управление платежами, оплата счетов, выписка чеков.
...
**Value outcomes:**
- Automated bill pay with approval workflow
- Multi-bank payment execution
- Payment reconciliation with GL entries
...
### Маршруты
- `/m/billpay-checks`
- `/m/billpay-checks/list`
- `/m/billpay-checks/payment/[id]`
...
**Payment**
- Поля: id, payee, amount, currency, dueDate, status, approvedBy, paidDate, bankRef
- Связи: через objectRef / foreignKey

**Invoice**
- Поля: id, vendor, amount, currency, dueDate, status, documentRef, paymentId
- Связи: через objectRef / foreignKey
...
- `POST /api/actions/m06/<actionKey>`
  - Вход: payload JSON
...
## 13. Demo Data
### Seed данные
- Файл: `src/db/seed/seed-module-06.json`
- Коллекции: `payments`, `invoices`, `paymentApprovals`, `bankAccounts`
