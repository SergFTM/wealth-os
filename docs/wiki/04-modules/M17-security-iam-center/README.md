# M17 Security and IAM Center

## 1. Назначение модуля
Управление безопасностью: RBAC, MFA, сессии, инциденты.
...
**Value outcomes:**
- Granular RBAC with scope-based access control
- MFA enforcement with multiple factors
...
**Role**
- Поля: id, name, permissions[], scope, isDefault, createdBy
- Связи: через objectRef / foreignKey

**SecurityIncident**
- Поля: id, type, severity, description, detectedAt, status, assignee, resolution
- Связи: через objectRef / foreignKey
...
- `POST /api/actions/m17/<actionKey>`
  - Вход: payload JSON
...
## 13. Demo Data
### Seed данные
- Файл: `src/db/seed/seed-module-17.json`
- Коллекции: `roles`, `roleBindings`, `permissions`, `mfaEnrollments`, `sessions`, `securityIncidents`, `securitySettings`, `accessRequests`
...
- **Sources:** All modules (auth)
- **Consumers:** M38 Audit, M17 Security
