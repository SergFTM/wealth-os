# M38 Audit and Compliance Pack

## 1. Назначение модуля
Единый audit trail и compliance readiness.
...
- Evidence pack assembly for regulatory audits
- SOC-style compliance readiness dashboard
...
**AuditEvent**
- Поля: id, timestamp, actor, action, objectRef, objectType, module, before, after, ipAddress
- Связи: через objectRef / foreignKey

**EvidencePack**
- Поля: id, name, scope, period, items[], status, generatedAt, generatedBy
- Связи: через objectRef / foreignKey
...
- `POST /api/actions/m38/<actionKey>`
  - Вход: payload JSON
...
## 13. Demo Data
### Seed данные
- Файл: `src/db/seed/seed-module-38.json`
- Коллекции: `auditEvents`, `auditLogViews`, `evidencePacks`, `accessReviews`, `reviewAttestations`
...
- **Sources:** All modules (audit events)
- **Consumers:** External auditors, Regulators
