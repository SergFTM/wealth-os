# M33 Trading Oversight and Order Management

## 1. Назначение модуля
Контроль торговых операций и исполнения ордеров.
...
**Value outcomes:**
- Pre-trade compliance checking against IPS constraints
- Order lifecycle tracking from initiation to settlement
- Execution quality analysis and broker comparison
...
**Order**
- Поля: id, securityId, side, quantity, orderType, limitPrice, status, placedBy, filledQty, avgPrice
- Связи: через objectRef / foreignKey

**Execution**
- Поля: id, orderId, execDate, quantity, price, broker, venue, commission, settlementDate
- Связи: через objectRef / foreignKey
...
- `POST /api/actions/m33/<actionKey>`
  - Вход: payload JSON
...
## 13. Demo Data
### Seed данные
- Файл: `src/db/seed/seed-module-33.json`
- Коллекции: `orders`, `executions`, `preTradeChecks`, `brokerAllocations`
...
- **Sources:** M09 IPS (constraints)
- **Consumers:** M21 Reconciliation, M02 GL
- **Governance:** M10 Risk
