# M24 Notes and Decision Log

## 1. Назначение модуля
Журнал заметок, решений и протоколов комитетов.
...
**Value outcomes:**
- Structured decision log with rationale and evidence
- Committee meeting minutes with voting records
...
**CommitteeDecision**
- Поля: id, meetingId, title, rationale, outcome, votesFor, votesAgainst, status
- Связи: через objectRef / foreignKey

**CommitteeMeeting**
- Поля: id, title, date, attendees[], agendaItems[], status, minutesRef
- Связи: через objectRef / foreignKey
...
- `POST /api/actions/m24/<actionKey>`
  - Вход: payload JSON
...
## 13. Demo Data
### Seed данные
- Файл: `src/db/seed/seed-module-24.json`
- Коллекции: `committeeDecisions`, `committeeMeetings`, `committeeVotes`, `committeeFollowUps`, `committeeAgendaItems`, `committeeTemplates`, `researchNotes`
...
- **Sources:** M09 IPS, M31 Deals
- **Consumers:** M38 Audit, M05 Documents
- **Governance:** M17 Security
